...
public interface Ensemble<E> extends Iterable<E>
{
	...
	/**
	 * Test d'appartenance d'un élément à l'ensemble en utilisant l'itérateur
	 * pour parcourir les éléments
	 * @param element l'élément dont on doit tester l'appartenance
	...
	public default boolean contient(E element)
	{
		for(Iterator<E> it=iterator();it.hasNext();)
		{
			if(it.next().equals(element)) /*ici== marche pas ,== compare aussi l'address*/
			{
				return true;
			}
		}
	...
	/**
	 * Test si ensemble est un sous-ensemble de l'ensemble courant. C'est à dire
	 * si l'ensemble courant contient tous les éléments de l'ensemble passé en
	 * argument
	 * @note Si l'ensemble passé en argument est null il ne sera pas considéré
	 * comme contenu.
	 * @param ensemble l'ensemble dont on veut tester s'il est un sous ensemble
	 * de l'ensemble courant
	 * @return true si ensemble est un sous-ensemble de l'ensemble courant,
	 * false sinon. false si ensemble est null.
	 */
	...
	/**
	 * Efface tous les élements de l'ensemble en utilisant le remove de
	 * l'itérateur fournit par {@link #iterator()}
	 */
	 ...
	 * @see ensembles.Ensemble#cardinal() Attention : si l'on utilise estVide
	 * dans cardinal, il ne faut pas utiliser cardinal dans estVide
	 * @note Cette méthode aura intérêt à être réimplémentée dans les classes
	 * filles qui utilisent des conteneurs pouvant donner leur taille
	 * directement
	 */
	...
	/**
	 * Implémentation de classe de l'union de deux ensemble dans un autre
	 * ensemble
	 * @param ens1 le premier ensemble
	 * @param ens2 le second ensemble
	 * @param res l'ensemble contenant l'union de ens1 et ens2
	 */
	 ...
	 * @param ens1 le premier ensemble
	 * @param ens2 le second ensemble
	 * @param res l'ensemble contenant l'intersection de ens1 et ens2
	 */
	 ...
	 * dans l'ensemble courant mais PAS dans l'ensemble passé en
	 * argument
	 */
	 ...
	 * @param ens1 le premier ensemble
	 * @param ens2 le second ensemble
	 * @param res l'ensemble contenant le complément de ens1 - ens2
	 */
	 ...
	 * L'ensemble correspondant à la différence symétrique contient les élément
	 * qui sont soit dans l'ensemble courant, soit dans l'autre ensemble mais
	 * pas dans les deux ensembles = (this - ensemble) union (ensemble - this)
	 * @param ensemble l'autre ensemble avec lequel on veut créer une différence
	 * symétrique
	 * @return un nouvel ensemble contenant la différence symmétrique de
	 * l'ensemble courant et de l'ensemble passé en argument
	 * @see ensembles.Ensemble#difference(ensembles.Ensemble)
	 */
	public default Ensemble<E> difference(Ensemble<E> ensemble)
	{
		/*
		 * TODO Remplacer par l'implémentation en utilisant
		 * 	- Soit (A - B) ∪ (B - A)
		 * 	- Soit (A ∪ B) - (B ∩ A)
		 */
	 ...
	 * @return une instance de la classe Class représentant le type des élements
	 * de l'ensemble si celui ci n'est pas vide, ou bien null si
	 * l'ensemble est vide.
	 * @note cette méthode sera utile dans l'implémentation de la méthode
	 * {@link #equals(Object)} pour déterminer si deux ensembles ont le
	 ...
	 * surchargées de la classe object (celles ci dépendant de l'état
	 * interne des objets, ce qui n'est pas le cas d'une interface)
	 */
	@Override
	 ...
	 * étant la somme des hascodes de ses éléments afin de ne pas tenir compte
	 * de l'ordre des éléments dans la collection sous-jacente.

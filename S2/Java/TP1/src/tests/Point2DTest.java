...
public class Point2DTest {
	...
	/** Constructeur de la classe de test. Initialise les attributs utilisés dans les tests
	 */
	public Point2DTest() {
		point = null;
	...
	public void testPoint2DPoint2D() {
		...
		Point2D specimen = new Point2D(randomNumber(maxRandom), randomNumber(maxRandom));
		assertNotNull(testName + " instance specimen", specimen);

		point = new Point2D(specimen);
		assertNotNull(testName + " instance copie", point);
		assertX(testName + ".getX() == " + specimen.getX(), specimen.getX(), 0.0);
		assertY(testName + ".getY() == " + specimen.getY(), specimen.getY(), 0.0);
	}
	...
	public void testGetNbPoints() {
		 ...
		 * du Garbage Collector. On peut donc juste vérifier qu'il y en a au moins un
		 */
	...
	public void testDeplace() {
		...
		Point2D retour = point.deplace(-deltaX, -deltaY);
		double tolerance = Point2D.getEpsilon();
		assertSame(testName + " return == point déplacé", point, retour);
	...
	public void testDistancePoint2DPoint2D() {
		...
		// Distances entre deux points diamétralement opposés le long d'un cercle
	...
	public void testEqualsObject() {
		...
		for(long i = 0; i < nbTrials; i++) {
			...
			otherPoint.deplace(radius * Math.cos(angle), radius * Math.sin(angle));
			double distance = point.distance(otherPoint);

			/* Attention, à cause des approximations dues aux cos et sin
			 * le déplacement peut être légèrement supérieur à epsilon
			 */
			if(distance < epsilon)
				assertEquals(testName + " point déplacé < epsilon [" + distance + "]", point, otherPoint);
			else
				assertFalse(testName + " point déplacé >= epsilon [" + distance + "]", point.equals(op));
		}
		...
		for(long i = 0; i < nbTrials; i++) {
			...
			if(distance < epsilon)
				assertEquals(testName + " point déplacé proche [" + distance + "]", point, otherPoint);
			else
				assertFalse(testName + " point déplacé loin [" + distance + "]", point.equals(otherPoint));
		}
	}
}

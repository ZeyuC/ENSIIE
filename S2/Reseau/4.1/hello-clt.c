...
int main(int argc, char** argv)
{
  prgname = argv[0];
  int statut;
  int duree=argc==3?0:atoi(argv[3])*100000; // en us
  ...
  if (duree<0)
    {
      fprintf(stderr,"tp doit etre positive ou null\n");
      exit(1);
    }
  ...
  //write(cx,"OK",3);
  //close(cx); 
  //exit(0);// pour que le serveur se tombe
  char PDU[100];
  /* Dialogue état début */
...
// signal(SIGPIPE,gest_sigpipe);  // recevoir et traiter la signal 
